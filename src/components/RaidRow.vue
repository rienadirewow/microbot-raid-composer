<template>
  <div
    class="bg-white rounded-lg shadow-sm border border-slate-200 p-3 hover:shadow-md transition-shadow"
  >
    <div class="flex items-center justify-between mb-2">
      <div class="flex items-center">
        <button
          @click="$emit('setCurrentPlayer', row.character.id)"
          class="mr-1 p-0.5 rounded-full transition-colors"
          :class="
            isCurrentPlayer
      ...
      <span class="text-xs text-slate-500 bg-slate-100 px-2 py-0.5 rounded-full">
        T{{ row.character.unlockedTiers.r }}R / T{{ row.character.unlockedTiers.d }}D
      </span>
    </div>
